...
func tutorial():
	...
	for __ in range(3):
		var p1 = add_platform_to_path(flat_scene, Directions.straight, 1)
		for i in p1.get_node("spawns").get_children():
	...
	add_item_to_path(firework_scene, p2.get_node("spawns").get_child(0).global_position)
